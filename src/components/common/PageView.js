...
class OrgModal extends Component{
	...
	render() {
		...
		var which;
		if(this.state.tab == 'Specials'){
			if(!this.props.deals.length){
				which = <Text style={styles.text}>There are currently no deals at {this.props.org.pageName}</Text>
			}else{
				which = <DealsList profile={this.props.profile} deals={this.props.deals} orgNameForBack={this.props.org.pageName}/>
			}
		}
		...
		return(
			<ScrollView style={{marginTop: this.props.references.globalMarginTop + 5}} contentContainerStyle={{alignItems: 'center',}}>
				...
				<TabBar icons={['Specials', 'Reviews', 'Jobs', 'Info']} changeTab={this.changeTab.bind(this)} isText={true} />
				{which}
			</ScrollView>
		);
	}
